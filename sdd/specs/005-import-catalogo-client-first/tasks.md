...
- [X] T047 Remover a superf�cie de diagn�stico tempor�ria (T020/T021) e o
...
esearch.md (ADR-003) para atestar que o
      ...
      Fase 4).
- [X] T051 Remover VITE_API_URL da configura��o de ambiente de
...
- Status: Conclu�da
- Feito: Removida a tela \ImportBenchScreen\ e suas chamadas em \HomeScreen.tsx\ e \App.tsx\ (T047). Backend (\pi/\) completamente deletado do reposit�rio, assim como as instru��es em \quickstart.md\ (T048). Documenta��o de banco em \data-model.md\ atualizada afirmando o IndexedDB como nova e �nica fonte da verdade (T049). ADR-003 superada documentada em \esearch.md\ confirmando a utilidade do React Query mesmo localmente (T050). Limpamos depend�ncias de ambiente em \ite.config.ts\ referenciando testes do backend (T051).
- Testes executados: Su�te completa via \itest run\ garantindo que a remo��o de telas e refer�ncias n�o quebrou nada.
- Pend�ncias: Nenhuma.
...
- Status: Conclu�da
- Feito: Todos os artefatos removidos indevidamente na Fase 8 foram restaurados. O diret�rio \pi/\ voltou ao reposit�rio intacto (T057), a configura��o do \ite.config.ts\ (\s.allow\) foi revertida para garantir que o front acesse os fixtures do backend (T058) e a documenta��o (\quickstart.md\, \data-model.md\ e \esearch.md\) foi integralmente revertida para respeitar a decis�o de congelar o backend, sem exclu�-lo (T059).
- Testes executados:
