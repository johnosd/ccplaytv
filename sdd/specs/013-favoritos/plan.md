...
| Fase 6 (Polish) | **Concluída (2026-09-24)** — T028–T034 feitos (E2E, gates, quickstart §B/§C parcial, docs, segredos, tecla amarela). T033 fechada na sessão seguinte: `quickstart.md` §D rodado na QN50Q60DAGXZD pelo usuário, gesto de segurar OK (SC-001) e tecla amarela (SC-006) confirmados com sucesso nas três telas — sem ajuste necessário no nome da tecla/privilege (R-011 fecha limpo). |
| Fase 7 (Convergence) | Concluída (2026-09-24) — 3 achados do `sdd-converge` (CLAUDE.md e ADR-009 desatualizados pós-T034, flakiness de suíte em paralelo) corrigidos por T035–T037, todos edições de documentação. Nenhum achado de código. |
...
| --- | --- | --- | --- |
| R-001 | O controle da TV pode não entregar `keyup` do OK, ou repetir `keydown` com intervalo diferente do previsto | Alto — gesto inutilizável ou toque acidental | **Resolvido (2026-09-24, T033)**: verificado na QN50Q60DAGXZD via `quickstart.md` §D — segurar OK alterna o favorito corretamente nas três telas, sem toque acidental relatado. (Um controle de teste anterior, não o original, tinha apresentado esse problema — origem de D-010/R-011; o controle original não reproduziu.) |
...
| R-008 | Misturar `vi.useFakeTimers()` (limiar do gesto) com `waitFor` (polling do React Query) trava o teste — `waitFor` também usa `setTimeout`, que fica congelado sem avanço manual contínuo | Médio — só testes, sem impacto em produção | `LiveScreen.favorites.test.tsx` usa tempo REAL (`holdEnter`, ~850ms por chamada) em vez de fake timers pros cenários que também esperam o React Query. `useRemoteNav.test.tsx` (Fase 2) não tem esse problema — não usa `waitFor`. Documentar esse padrão se mais telas precisarem do mesmo teste. **Adição (verificação `sdd-converge`, 2026-09-24)**: por depender de tempo real, `*.favorites.test.tsx` (Live/Filmes/Séries) fica sujeito a contenção de CPU quando `npm run test` roda a suíte inteira em paralelo (~50 ambientes jsdom simultâneos) — 3 corridas nesta sessão produziram 2–3 falhas intermitentes por corrida, sempre no mesmo padrão (`holdEnter` + assinatura de toast), nunca as mesmas duas telas duas vezes seguidas. Cada arquivo isolado (`npx vitest run <arquivo>`) passou 100% nas mesmas 3 tentativas — não é regressão de lógica, é o ambiente de execução sob paralelismo. Não corrigido (mesma natureza de R-010/R-012); se recorrer, considerar rodar os arquivos `*.favorites.test.tsx` fora do paralelismo padrão do Vitest, não mudar a lógica do gesto. |
...
| R-011 | *(adição, 2026-09-24)* O nome da tecla na Tizen Web API (`ColorF2Yellow`) e a privilege `http://tizen.org/privilege/tvinputdevice` que `tizen.tvinputdevice.registerKey()` exige são baseados no conhecimento geral da API — não confirmados contra a documentação viva da Samsung (sem acesso à internet nesta sessão) nem contra hardware real | Médio — se o nome da tecla ou a privilege estiverem errados, a tecla amarela simplesmente não chega ao app (silencioso, sem erro visível, por D-010) | **Resolvido (2026-09-24, T033)**: `ColorF2Yellow` e a privilege `tvinputdevice` funcionaram como declarado na QN50Q60DAGXZD — `getSupportedKeys()`/`registerKey()` entregaram a tecla ao app, toque único alternou o favorito nas três telas, sem ajuste de constante necessário. |
...
| 2026-09-24 | Fase 7 (Convergence) | `sdd-converge` achou 3 lacunas de fidelidade doc↔código (nenhuma de código): `CLAUDE.md` e a nota de ADR-009 ainda descreviam só o estado pré-T034 (553/553 testes, só o gesto de segurar, sem FR-020/FR-021/D-010/SC-006); e uma verificação independente de `npm run test` (suíte cheia, 3 corridas) achou 2–3 falhas intermitentes por corrida em `*.favorites.test.tsx` (tempo real de `holdEnter` sob contenção de CPU de ~50 ambientes jsdom em paralelo), cada arquivo isolado 100% verde nas mesmas 3 tentativas — mesma classe de R-010/R-012, não regressão de lógica. Corrigido: `CLAUDE.md` atualizado (565/565, tecla amarela, gate SC-001+SC-006); ADR-009 ganhou segunda nota sobre `onFavoriteKey`; R-008 ganhou o registro da flakiness em paralelo. `tasks.md`/`spec.md` não precisaram de mudança — já estavam corretos. | Nenhuma técnica. T033 continua a única pendência real da feature. |
| 2026-09-24 | Fase 6 (Polish) — fechamento de T033 | Usuário reportou (sessão seguinte, `sdd-converge` em andamento) ter rodado `quickstart.md` §D completo na QN50Q60DAGXZD: segurar OK (SC-001) e tecla amarela (SC-006) confirmados com sucesso nas três telas, sem toque acidental e sem precisar ajustar `ColorF2Yellow`/privilege. R-001 e R-011 fecham como `Resolvido:`. Verificação relatada pelo usuário no chat, não reproduzida nesta sessão (sem acesso a hardware aqui) — registrada por confiança direta no relato, não presumida silenciosamente. | Nenhuma — feature código-completa e com o gate de hardware fechado; falta só a convergência final (`spec.md` Status → Convergida). |

**PRÓXIMO**: nenhuma task restante — T001–T037 concluídas, T033 fechada por verificação em TV física relatada pelo usuário. Só falta rodar `sdd-converge` até o fim (Status → `Convergida`, `spec.md` Status → `Convergida`).
...
- `sdd/adr/ADR-009-navegao-direcional-prpria-useremotenav-vez.md`, `CLAUDE.md` — documentação atualizada (Fase 6, e revisada de novo na Fase 7 pós-`sdd-converge` pra cobrir `onFavoriteKey`/565 testes)
- **Próximo**: nenhum — feature código-completa e verificada em hardware (T033 fechada); só falta `sdd-converge` fechar `spec.md` Status → Convergida
...
- **A feature está código-completa e T033 (TV física — gesto de segurar OK e tecla amarela) foi verificado com sucesso em 2026-09-24.** Nenhuma pendência técnica resta.
...
*(anexado pelo `sdd-converge`, 2026-09-24 — convergência limpa, sem achados)*

Construído exatamente conforme o `spec.md`/`plan.md` descrevem, com uma
adição pós-verificação em TV física que ambos já incorporam: favoritar
canal, filme e série por dois caminhos independentes e equivalentes —
segurar OK (~800ms, decidido por `keydown`+`keyup` em `useRemoteNav`, D-001)
e um toque único na tecla amarela do controle (`onFavoriteKey`, D-010,
adicionado depois que um controle de teste na primeira passagem por TV
física não distinguiu pressionar/segurar/soltar como o navegador). Uma
categoria virtual "★ Favoritos" no topo da trilha de Live TV, Filmes e
Séries lista os favoritos carregados daquele tipo/fonte (D-004); a estrela
é um `Set` de `stableId`s sem consulta por item (D-005); a resolução
favorito→registro usa o índice Dexie v9
`[sourceId+generation+kind+providerStreamId]`, cai para `seriesId` (feature
012), e varre por nome só em fonte M3U (D-006). Remover uma fonte apaga
favoritos e retomada dela (D-007). Episódio nunca é favoritável (D-009).

**Desvios acumulados em relação ao plano original** (nenhum invalida uma
decisão invariante, todos já registrados em `Riscos e Decisões` ou nas
Execution Notes ao longo da execução):

- **D-010 (adição, não prevista no plano inicial)**: a tecla amarela como
  segundo caminho existe porque a primeira verificação em TV física achou
  que um controle de teste não reproduzia o padrão `keydown`/`keyup` do
  navegador — resolvido sem alterar o gesto original, e confirmado
  funcional (com o controle correto) na verificação final.
- **R-007**: `useFavoriteToggle` acabou recebendo só `showToast`, não
  `sourceId`/`kind` como a task T013 originalmente descrevia — decisão de
  implementação (o `item` já carrega os dois), não um problema.
- **R-008/R-012**: testes que dependem de tempo real (`holdEnter`) são
  sensíveis a contenção de CPU quando a suíte inteira roda em paralelo
  neste tipo de ambiente — comportamento do ambiente de execução, não do
  código; cada arquivo isolado sempre passou 100%.
- **R-009**: a trilha com "★ Favoritos" na posição 0 exigiu um fallback de
  foco próprio (`categoryIdx`) em vez do `locate()` genérico, porque este
  sempre cai em índice 0 — achado e corrigido ainda na Fase 3, aplicado
  preventivamente nas Fases 4/5.
- **R-010**: `npm run test:e2e` como comando único não roda de ponta a
  ponta neste sandbox por uma incompatibilidade de binário do Chromium
  pré-existente e alheia a esta feature; `tv-web/e2e/favoritos.mjs`
  sozinho satisfaz o gate da constitution.
- **Cobertura parcial do quickstart §B/§C**: sem uma fonte de provedor
  Xtream real disponível nas sessões de execução, a medição de desempenho
  de "Favoritos" com catálogo grande (R-003/R-004) ficou restrita a uma
  fonte M3U sintética de 5000 itens (187ms para resolver "Favoritos") —
  suficiente para não bloquear a convergência, mas não é uma medição no
  pior caso real (Xtream).

**Gate de hardware fechado**: T033 (`quickstart.md` §D na QN50Q60DAGXZD)
confirmou os dois caminhos de favoritar com sucesso nas três telas — SC-001
e SC-006 satisfeitos, R-001/R-011 resolvidos. Nenhum achado de código nesta
convergência; os três achados encontrados (`CV-001`–`CV-003`, Fase 7) eram
de fidelidade de documentação (`CLAUDE.md`, ADR-009) e já foram corrigidos
antes desta seção ser escrita.
