...
| Fase 6 (Polish) | Quase concluída — T028–T032 feitos (E2E, gates, quickstart §B/§C parcial, docs, segredos). **Só falta T033 (TV física, gate de SC-001/R-001)**, sem acesso à TV nesta sessão |
...
**PRÓXIMO**: T033 — `npm run build:tizen` + instalar via skill `tizen-tv` + `quickstart.md` §D na QN50Q60DAGXZD, cobrindo SC-001 (20 OK curtos + 20 demorados alternados de verdade no controle físico) e SC-002/SC-004. Até lá, a feature está código-completa mas não pode ser convergida sem essa verificação ou uma decisão explícita do usuário para fechar sem ela (constitution, exceção de "Validação em hardware real" — mesmo padrão da feature 011).
...
- `tv-web/e2e/favoritos.mjs` + `e2e/fixtures/favoritos.m3u` — roteiro E2E (Fase 6, feito)
- `sdd/adr/ADR-009-navegao-direcional-prpria-useremotenav-vez.md`, `CLAUDE.md` — documentação atualizada (Fase 6, feita)
- **Próximo**: nenhum arquivo de código — só o ciclo `tizen-tv` (T033) na TV física
...
- **A feature está código-completa.** Tudo que falta é T033 na TV física — não há mais nenhum arquivo de código a mudar antes disso. Ao retomar, ir direto para o skill `tizen-tv`.
