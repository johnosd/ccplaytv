...
**PRÓXIMO**: Fase 4 (US2) — `MoviesScreen.tsx`/`SeriesScreen.tsx` (T020–T023) recebem o mesmo tratamento da Fase 3: trilha com "★ Favoritos", gesto de OK na grade, estrela, dica, conteúdo de Favoritos — atenção a R-009 (mesmo cuidado de fallback de foco) e ao fato de que o gesto na grade concorre com o `onOpenMovie`/detalhe no OK curto.

## Arquivos Principais

<!-- Sobrescrita a cada checkpoint — foco da etapa atual, não a árvore inteira. -->

- `tv-web/src/features/live/LiveScreen.tsx` — trilha, gesto, estrela, conteúdo de Favoritos (Fase 3, feita)
- `tv-web/src/features/live/LiveScreen.favorites.test.tsx` — novo, 11 testes (a)-(k)
- `tv-web/src/features/live/LiveScreen.test.tsx` — `press()` ajustada; 1 asserção atualizada
- `tv-web/src/features/movies/MoviesScreen.tsx` — próximo arquivo a mudar (Fase 4)
- `tv-web/src/features/series/SeriesScreen.tsx` — depois de Filmes (Fase 4)
...
- **`locate()` genérico não serve pra trilha com Favoritos**: seu fallback é sempre índice 0, que agora é a entrada virtual. Qualquer tela nova com essa trilha (Filmes, Séries) precisa do mesmo fallback próprio que `LiveScreen.tsx` usa (R-009), não o `locate()` direto.
